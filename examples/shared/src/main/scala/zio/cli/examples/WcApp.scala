...
import zio.{Console, Has, URIO, ZIO, ZIOAppDefault}
...
object WcApp extends ZIOAppDefault {
  ...
  val execute: (WcOptions, ::[Path]) => URIO[Has[Console], Unit] = {
    def printResult(res: List[WcResult]): ZIO[Has[Console], Nothing, Unit] = {
    ...
    (opts, paths) => {
      printLine(s"executing wc with args: $opts $paths").! *>
        ZIO
          .foreachPar[Any, Throwable, Path, WcResult, List](paths) { path =>
            ...
            val zippedSinks: ZSink[Any, Nothing, Byte, Byte, (Option[Long], Option[Long], Option[Long], Option[Long])] =
              (byteCount <&> lineCount <&> wordCount <&> charCount).map(t => (t._1, t._2, t._2, t._2))
